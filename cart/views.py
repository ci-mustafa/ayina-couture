...
def add_to_cart(request, id):
    ...
    size and color selection. If the product already exists in the cart, its quantity 
    is incremented; otherwise, a new entry is created. The cart is saved in the session, 
    ensuring that the user's cart persists across requests.

    Args:
        request (HttpRequest): The HTTP request object containing POST data (such as product quantity, size, and color).
        id (str): The ID of the product to be added to the cart.

    Returns:
        HttpResponseRedirect: Redirects the user to the specified URL (usually the cart page or product page).

    Behavior:
        - If the product has sizes, a unique key (product_id + size) is used to identify the cart item.
        - If the product has colors, the selected color is included in the key.
        - If the product already exists in the cart, its quantity is updated by adding the quantity from the POST request.
        - If the product is not in the cart, a new entry is created with the specified quantity, size, and color (if applicable).
        - The cart is stored in the session under the 'cart' key.
        - A success message is shown to the user after the product is added.

    Notes:
        - If the product does not have a size or color, those fields are omitted from the cart item.
        - If the user attempts to add a product to the cart that has color options, the color is required to be passed in the POST data.
        - If no redirect URL is provided in the POST data, the user will be redirected to the same page they came from.

    Example:
        - Product has no size or color: Cart item key will be the product ID.
        - Product has size but no color: Cart item key will be the product ID and size.
        - Product has both size and color: Cart item key will be the product ID, size, and color.
    ...
    # Create a unique key by combining the product id, size (if applicable), and color (if applicable)
...
def update_cart_item(request, id, size=None, color=None):
    ...
    This view allows users to update the quantity, size, and color of an item already in their shopping cart.
    It retrieves the product using its ID, checks if the item exists in the cart, and processes the update.

    Args:
        request (HttpRequest): The HTTP request object containing the cart and form data.
        id (str): The ID of the product to be updated in the cart.
        size (str, optional): The size of the item to update, if applicable. Defaults to None.
        color (str, optional): The color of the item to update, if applicable. Defaults to None.

    Returns:
        HttpResponse: Redirects to the cart view upon successful update, or renders the update page if the item is found.

    Raises:
        Http404: If the product is not found in the database (via `get_object_or_404`).
    """
    ...
    return render(request, 'cart/update_cart_item.html', {
        'product': product,
        'cart_item': cart.get(old_cart_item_key),  # Pass the correct cart item to the template
        'size': size,
        'color': color,
    })
...
def delete_cart_item(request, id, size=None, color=None):
    ...
    This view deletes a specific item from the cart, either with or without a size or color.
    ...
    Args:
        ...
        color (str, optional): The color of the item (if applicable). Defaults to None.
